...
Finance is one of fields where machine learning is commonly used. It deals with a huge amount of data and is surrounded by a lot of uncertainties. To predict future, ranging from stock market prices and sales of retailers to individual repayment of a debt, using pre-existing data is a crucial part of finance. In this paper, we will deal with a problem many credit card companies are facing. Can we predict whether a credit client will pay their debt or fail to pay? Traditionally, finding a credible borrower have been a kind of know-how, or skill and experience nurtured by experts. Instead, we try to form machine learning models, using a dataset which is open to public.
...
```{r unique values SEX, echo=FALSE}
sort(unique(original_default$SEX))
```

"1", "2" correspond to male and female respectively[^13]. It is categorical data. Male is approximately 40% and female is 60%.
...
```{r unique values EDUCATION, echo=FALSE}
sort(unique(original_default$EDUCATION))
```

It is categorical variable. The numbers indicate clients' final education. They have meanings as follows ; 1=graduate school, 2=university, 3=high school, 4=others, 5=unknown, 6=unknown[^14]. In the description 0 is not defined. We draw a distribution and a default rates proportion graph of this variable.
...
```{r unique values MARRIAGE, echo=FALSE}
sort(unique(original_default$ MARRIAGE))
```

They are also categorical data. The value number means clients' marital status[^15]. 1=married, 2=single, 3=others. This column includes undefined 0.
...
Comparing married and single people, married people are a little more likely to default.

### 6 "AGE"

It is age in years[^16]. As its name indicates, it is numerical data.
...
The data description says; "-1"= pay duly, "1"= payment delay for 1 month, "2" = payment delay for 2 months, ... 9 = payment delay for 9 months and above. "0" and "-2" are not defined. And no 9.
...
Maximum values in each group are approximately 950000, except BILL_AMT3. And medians and means are decreasing from BILL_AMT1 to BILL_AMT6. Likewise previous variables, we plot BILL_AMT distribution filling each bar with its default rate.
...
It is right skewed significantly. Maximum amount is more than 850,000 NT dollars, but its mean and median are 5664 and 2100 respectively. Moreover, approximately 17% of clients' values are 0.
...
As such, the credit company would falsely give credit to a lot of clients who will fail to repay a debt. The loss for the company would be huge. To avoid this, we need to consider specificity. In this sense, balanced accuracy is a good metric to evaluate. Therefore, our goal is to pursue more accurate accuracy than guessing all responses are 0, taking account of improving specificity. We will use both *accuracy* and *balanced accuracy* to evaluate each model.
...
```{r confusion matrix of glm fewer model, echo=FALSE, message=FALSE, warning=TRUE}
...
$\hat{p}_{j,k}$ as the proportion of observations in partition $\ {j}$ that are of class $\ {k}$.[^21] If there are lots of classes in the partition, Gini index, also called Gini impurity, increases up to 1. On the other hand, if there are few classes in the partition it decreases down to 0. If you predict the result perfectly, the index is 0.
...
```{r comaprison decision tree and regression 1, echo=FALSE}
...
```{r comaprison decision tree and regression 2, echo=FALSE}
bind_rows(rpart_tuned_results, rpart_results ,glm_s_results, glm_f_results) %>% knitr::kable()
...
```{r comparison random forest, decision trees, and logisitic regression, echo=FALSE}
bind_rows(rf_results, rpart_tuned_results, rpart_results ,glm_s_results, glm_f_results) %>% knitr::kable()
```
...
```{r echo=FALSE}
bind_rows(glm_s_results, glm_f_results, rpart_results, rpart_tuned_results, rf_results, rf_cv_results)%>% knitr::kable()
```

Among them, the best performance in terms of both accuracy and balanced accuracy is produced by "**random forest model**".

Here we show variable's importance in this model again.

```{r importance of random forest model variables 2, echo=FALSE, fig.height=3, fig.width=6}
imp_df <- data.frame(Variable = names(rf_mdl$variable.importance),
                   Importance = as.numeric(rf_mdl$variable.importance)) 
...
As we mentioned before, **PAY_0** is the most important variables compared with others. Then follows **BILL_AMT1**, **PAY_AMT1**, and **LIMIT_BAL**, and **AGE.** Recent financial conditions of clients are crucial to predict the result. SEX and MARRIAGE are not important.
