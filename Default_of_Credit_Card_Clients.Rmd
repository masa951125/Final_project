...
```{r FIrst 6 rows of the data, echo=FALSE}
...
```{r echo=FALSE}
plot_correlation(original_default) +theme(axis.text=element_text(size=6))+coord_fixed(ratio = 0.8)
```
...
First, we look into the outcome, "default.payment.next.month". The data description says, "Default payment, 1=yes, 0=no.[^9]" We draw a distribution graph.

[^9]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r outcome graph, echo=FALSE}
...
```{r SEX stacked bar graph, echo=FALSE}
original_default %>% ggplot(aes(x=SEX, fill= DEFAULT)) +
...
In this variable, values are "1", "2", "3", "4", "5", "6". They are categorical values. The numbers have meanings as follows ; 1=graduate school, 2=university, 3=high school, 4=others, 5=unknown, 6=unknown[^12]. We plot its distribution.
...
```{r EDUCATION stacked bar graph, echo=FALSE}
original_default %>% ggplot(aes(x=as.factor(EDUCATION), fill= DEFAULT)) +
  geom_bar(position="fill") +
  ggtitle("EDUCATION")
```
