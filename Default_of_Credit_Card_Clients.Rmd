...
In this variable, values are "1", "2", "3", "4", "5", "6". They are categorical values. The numbers have meanings as follows ; 1=graduate school, 2=university, 3=high school, 4=others, 5=unknown, 6=unknown[^12]. We draw a distribution and a default rates proportion graph of this variable.
...
This is also categorical data. The value number means clients' marital status[^13]. 1=married, 2=single, 3=others. But this column includes undefined 0. There are 54 individuals whose values are 0.
...
This group includes variable from PAY_0, PAY_2 \~PAY_6[^14]. PAY_0 means repayment status in September, 2005. Then go back in time by a month until April, 2005, PAY_6. Their values are;
...
This variable means an amount of bill statement. BILL_AMT1 is a record in September, 2005[^15]. Then go back in time by a month until April, 2005.
...
This variable means an amount of previous payment. PAY_AMT1 is an amount of previous payment in September, 2005[^16]. Likewise BILL_AMT, PAY_AMT goes back in time by a month from August to April, 2005 which is PAY_AMT6.
...
[^19]: Irizarry, Rafael A, "Introduction to Data Science: Data Analysis and Prediction Algorithms with R, 31.10.4 Classification (decision) trees". *Internet archive*, <https://rafalab.github.io/dsbook/examples-of-algorithms.html#classification-decision-trees>
...
Random forest algorithm showed the best performance. Fitting it to the test set, we got accuracy 0.8226962 and balanced accuracy 0.6471006. But when tuned, the model showed over-fitted results. As well as the random forest models, the default decision tree model showed better performance than the tuned model.
