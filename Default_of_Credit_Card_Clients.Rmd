...
First, we need to check the downloaded dataset. It has 30000 rows and 25 columns.

```{r original_default dimention, include=FALSE}
dim(original_default)
```

We look into the data using "str" and "summary" function.

```{r Data exploration (str), echo=FALSE, echo=FALSE}
...
```{r Correlation Matrix, fig.height =7, fig.width=7,echo=FALSE}
...
This is an "amount of given credit in NT dollars (includes individual and family/supplementary credit)[^10]" It is clearly numeric data.
...
```{r LIMIT_BAL graph, fig.height= 5, fig.width= 7, fig.align="center", echo=FALSE}
...
```{r SEX distribution,fig.height= 5, fig.width= 5, fig.align="center", echo=FALSE}
...
```{r SEX stacked bar graph,fig.height= 5, fig.width= 5, fig.align="center", echo=FALSE}
original_default %>% ggplot(aes(x=SEX, fill= DEFAULT)) +
...
```{r EDUCATION distribution,fig.height= 5, fig.width= 7, fig.align="center", echo=FALSE}
...
```{r EDUCATION stacked bar graph, fig.height= 5, fig.width= 7, fig.align="center", echo=FALSE}
original_default %>% ggplot(aes(x=as.factor(EDUCATION), fill= DEFAULT)) +
  geom_bar(position="fill") +
  ggtitle("EDUCATION")
```
...
marital status. 1=married, 2=single, 3=others. Categorical data. Plot. Stack bar graph

```{r MARRIAGE distribution, fig.height= 5, fig.width= 7, fig.align="center", echo=FALSE}
...
```{r MARRIAGE stacked bar graph, fig.height= 5, fig.width= 7, fig.align="center",echo=FALSE}
original_default %>% ggplot(aes(x=as.factor(MARRIAGE), fill= DEFAULT)) +
  geom_bar(position="fill") +
  ggtitle("MARRIAGE")
```
...
ggplot(data=original_default, aes(AGE, fill=DEFAULT)) +geom_histogram(bins=50)
```
