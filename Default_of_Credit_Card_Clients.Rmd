...
#confusionMatrix(in caret package) is a function to produce confusion matrix and metrics such as accuracy 
```
...
In the step-wise regression, we make two models, null model with no predictors and full model with all predictors. Then we use "step" function to conduct step-wise regression.
...
The step-wise regression selected the formula as follows;
...
Bearing these in mind, we narrow down features and leave out BILL_AMT2 to BILL_AMT6 and PAY_AMT2 to PAY_AMT6. Then we predict responses using glm function.
...
In this model, formula is as follows;

```{r echo=FALSE}
glm_fewer_mdl$formula
```
...
A decision tree is a familiar and intuitive method. For example, if you feel sick, a physician may ask you questions following a tree chart. You are asked whether it is yes or no at each node of the chart. Then the doctor gets your health outcome.
...
[^17]: Rafael A.Irizarry, 2021, *Introduction to Data Science,* 31.10.4 Classification (decision) trees <https://rafalab.github.io/dsbook/examples-of-algorithms.html#classification-decision-trees>

We use "rpart" function to make a model, then predict the outcome. Confusion matrix is;
...
Compared with the previous logistic regression model, its accuracy has improved but its balanced accuracy become worse.
...
We train the dataset using "train" function.
