...
We use a dataset stored in Kaggle[^9]website. In the description, it says, "This dataset contains information on default payments, demographic factors, credit data, history of payment, and bill statements of credit card clients in Taiwan from April 2005 to September 2005." It is CSV file.
...
[^10]: <https://github.com/masa951125/Final_project/raw/main/UCI_Credit_Card.csv>
...
```{r str original_default, echo=FALSE}
...
```{r Correlation Matrix, echo=FALSE, fig.height=6, fig.width=6}
...
First, we look into the target variable, "default.payment.next.month". The data description says, "Default payment, 1=yes, 0=no.[^11]" We show the proportion of "0","1".

[^11]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r proportion of values in the target variable, echo=FALSE}
...
```{r change the name of the target variable, include=FALSE}
n <-which(names(original_default)=="default.payment.next.month")
names(original_default)[n] <- "DEFAULT"
```

```{r factoring the target variable, include=FALSE}
...
```{r summary LIMIT_BAL, echo=FALSE}
...
The values "1", "2" correspond to male and female respectively[^13]. Male is approximately 40% and female is 60%.

[^13]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r proportion of SEX, echo=FALSE}
prop.table(table(original_default$SEX))
```

We draw a distribution and a default rates proportion graph of this variable.

```{r SEX graphs, echo=FALSE, fig.align="center", fig.height=3, fig.width=6}
...
In this variable, values are "1", "2", "3", "4", "5", "6". They are categorical values. The numbers have meanings as follows ; 1=graduate school, 2=university, 3=high school, 4=others, 5=unknown, 6=unknown[^14]. We draw a distribution and a default rates proportion graph of this variable.

[^14]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r EDUCATION graphs, echo=FALSE, fig.align="center", fig.height=3, fig.width=6}
...
This is also categorical data. The value number means clients' marital status[^15]. 1=married, 2=single, 3=others. But this column includes undefined 0. There are 54 individuals whose values are 0.

[^15]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r Number of 0 value clients, include=FALSE}
sum(original_default$MARRIAGE==0)
```

We draw a distribution and a default rates proportion graph of this variable.

```{r MARRIAGE graphs, echo=FALSE, fig.align="center", fig.height=3, fig.width=6}
...
This group includes variable from PAY_0, PAY_2 \~PAY_6[^16]. PAY_0 means repayment status in September, 2005. Then go back in time by a month until April, 2005, PAY_6. Their values are;

[^16]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r unique values of PAY, echo=FALSE}
...
```{r PAY_0 graphs, echo=FALSE, fig.height=6, fig.width=6}
...
PAY_2 \~ PAY_6 's distributions are almost as the same as PAY_0. We show their distributions.
...
This variable means an amount of bill statement. BILL_AMT1 is a record in September, 2005[^17]. Then go back in time by a month until April, 2005.

[^17]: <https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset>

```{r summary of BILL_AMT, echo=FALSE}
summary(original_default[,13:18]) 
```

This is numerical variable. Maximum values are fluctuating, but medians and means are decreasing from BILL_AMT1 to BILL_AMT6. Likewise previous variables, we plot BILL_AMT distribution filling each bar with its default rate.
...
This variable means an amount of previous payment. PAY_AMT1 is an amount of previous payment in September, 2005[^18]. Likewise BILL_AMT, PAY_AMT goes back in time by a month from August to April, 2005 which is PAY_AMT6.
...
This is numerical data. Their maximum values are fluctuating, but medians are gradually decreasing from PAY_AMT1 to PAY_AMT6.

We plot PAY_AMT1 distribution filling each bar with its default rate.

```{r PAY_AMT1 graph, echo=FALSE, fig.height=3, fig.width=6, message=FALSE}
...
```{r proportion of 0 PAY_AMT1, echo=TRUE}
...
```{r removal of ID column, include=FALSE}
...
```{r factorizaion of variables, include=FALSE}
...
```{r scaling of numric variables, include=FALSE}
...
```{r spliting test set and pre train set, include=FALSE}
...
```{r spliting train set and validation set, message=FALSE, include=FALSE}
set.seed(2021, sample.kind = "Rounding")
...
```{r datasets proportion of 0,1, echo=FALSE}
...
```{r confusion matrix all values are 0, echo=FALSE}
...
```{r metrics of all values are 0, include=FALSE}
...
```{r glm_fewer model, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r summary of glm fewer model, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r confusion matrix of glm fewer model, echo=TRUE, message=FALSE, warning=TRUE}
...
```{r statistic metrics of glm fewer model, echo=FALSE}
...
```{r glm step-wise model, message=FALSE, warning=FALSE, include=FALSE}
...
```{r summary of glm step-wise model, echo=FALSE}
summary(step_mdl)
```
...
```{r confusion matrix of glm step-wise model, echo=FALSE, warning=TRUE}
...
```{r statistic metrics of glm step-wise model, echo=FALSE}
...
```{r table of two regression model, echo=FALSE}
...
$\hat{p}_{j,k}$ as the proportion of observations in partition $\ {j}$ that are of class $\ {k}$.[^20] If there are lots of classes in the partition, Gini index, also called Gini impurity, increases up to 1. On the other hand, if there are few classes in the partition it decreases down to 0. If you predict perfectly, the index is 0.

[^20]: Irizarry, Rafael A, "Introduction to Data Science: Data Analysis and Prediction Algorithms with R, 31.10.4 Classification (decision) trees". *Internet archive*, <https://rafalab.github.io/dsbook/examples-of-algorithms.html#classification-decision-trees>
...
```{r content of rpart model, echo=FALSE}
rpart_mdl
```

We draw a decision tree based on this model.

```{r decision tree of rpart model, echo=FALSE, fig.height=3, fig.width=6}
...
```{r prediction of rpart model, include=FALSE}
rpart_pred <- predict(rpart_mdl, validation_set, type="class")
```

```{r confusion matrix of rpart model, echo=FALSE}
confusionMatrix(rpart_pred, validation_set$DEFAULT)$table
```

Statistic metrics;

```{r statistic metrics of rpart model, echo=FALSE}
...
```{r rpart tunable parameters, echo=FALSE}
modelLookup("rpart")
```
...
```{r cp of rpart model, eval=FALSE, include=FALSE}
...
```{r rpart tuned model, include=FALSE}
set.seed(2021, sample.kind = "Rounding")
...
```{r content of rpart tuned model, echo=FALSE, fig.height=3, fig.width=6}
plot(rpart_tuned_mdl)
```

Then draw a decision tree.

```{r decision tree of rpart tuned model, echo=FALSE, fig.height=3, fig.width=6}
...
```{r confusion matrix of rpart tuned model, echo=FALSE}
...
```{r statistic metrics of rpart tuned model, echo=FALSE}
...
Decision tree algorithm is easy to understand as it follow human decision making process. However, it "can easily over-train" and "is not very flexible."[^21] To improve this, we introduce random forest algorithm.
...
Random forest is one of the most popular machine learning algorithms. As its name implies, it makes multiple trees "**randomly** different" from a dataset, then produce a final prediction based on the average prediction of "combination of trees", namely "**forest**".[^22]

[^22]: Irizarry, Rafael A, "Introduction to Data Science: Data Analysis and Prediction Algorithms with R, 31.11 Random forests". *Internet archive*, <https://rafalab.github.io/dsbook/examples-of-algorithms.html#random-forests>
...
```{r content of random forest model, echo=FALSE}
rf_mdl
```

What kind of variables are important to from this model? Plot the variables' importance;

```{r importance of random forest model variables, echo=FALSE, fig.height=3, fig.width=6}
...
```{r confusion matrix of random forest model, echo=FALSE}
...
```{r statistic metrics of random forest model, echo=FALSE}
...
The results has been much improved, especially specificity, compared to other models. Can we improve the result further? Using "modelLookup", we can see parameters which can be tuned.

```{r ranger tunable parameters, echo=FALSE}
modelLookup("ranger")
```
...
```{r random forest tuned model, include=FALSE}
set.seed(2021, sample.kind = "Rounding")
...
```{r content of random foreset tuned model, echo=FALSE}
rf_cv_mdl
```

The mtry which produces the highest accuracy is 7. Plot mtry and accuracy.

```{r graph of mtry and accuracy, echo=FALSE, fig.height=3, fig.width=6}
plot(rf_cv_mdl)
```

Likewise the simple random forest mode, plot the variables' importance;

```{r importance of random forest tuned model variables, echo=FALSE, fig.height=10, fig.width=6}
...
Overall, the same trend is observed in the tuned random forest model. Whether PAY_0 value is -1(the second level) is important to determine the performance of the model.

Then fit the model to the validation set and show its confusion matrix;

```{r confusion matrix of random forest tuned model, echo=FALSE}
...
```{r statistic metrics of random foreset tuned model, echo=FALSE}
...
```{r comparison of the two random forest model, echo=FALSE}
